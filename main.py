...
def tip():
    if (Whether_tip != False):
        try:
            ...
            windsc = data["newslist"][0]["windsc"]  # 风力等级
            vis = data["newslist"][0]["vis"]  # 能见度
            uvindex = data["newslist"][0]["uv_index"]  # 紫外线强度
...
def aqi():
    if (Whether_aqi != False):
        try:
            conn = http.client.HTTPSConnection('api.tianapi.com')  # 接口域名
            params = urllib.parse.urlencode({'key': tianxing_API, 'area': city})
            headers = {'Content-type': 'application/x-www-form-urlencoded'}
            conn.request('POST', '/aqi/index', params, headers)
            res = conn.getresponse()
            data = res.read()
            data = json.loads(data)
            quality = data["newslist"][0]["quality"]  # 空气质量
            return quality
        except:
            return ("空气质量API调取错误，请检查API是否正确申请或是否填写正确")
...
if __name__ == "__main__":
    ...
    # 是否启用空气质量API
    Whether_aqi = config["Whether_aqi"]
    # 获取星座    #是否启用健康小提示API
    ...
    zwxtips = zwxtips()
    # 自定义提示内容
    wenhou = str('亲爱的乖宝宝，早上好！记得按时吃早饭午饭晚饭啊，今天也要开心哦')
